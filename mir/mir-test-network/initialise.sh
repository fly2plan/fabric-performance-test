...
./network.sh $1
if [ "$1" == "down" ]; then
    echo "Removing all stopped container"
    docker system prune
    ...
    peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C mychannel -n basic --peerAddresses localhost:7051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt" --peerAddresses localhost:9051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt" -c '{"function":"emptyContract","Args":[]}'
    echo "Confirm the assets were added to the ledger"
    peer chaincode query -C mychannel -n basic -c '{"Args":["emptyContract"]}'
fi
